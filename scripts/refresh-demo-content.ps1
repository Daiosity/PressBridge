...
$seedScript = Join-Path $projectRoot 'scripts\seed-demo-content.php'
$sitesFile = Join-Path $env:APPDATA 'Local\sites.json'
$tempIni = Join-Path $projectRoot '.tmp-local-php.ini'
$outputFile = Join-Path $projectRoot 'build\demo-content-map.json'

Assert-Condition (Test-Path $seedScript) "Demo seed script not found at $seedScript"
...
try {
    ...
    $buildDir = Split-Path -Parent $outputFile
